...
func (r *serviceReconciler) reconcile(ctx context.Context, req reconcile.Request) error {
	...
	r.metricsCollector.ObserveControllerReconcileLatency("service", "fetch_service", fetchServiceFn)
	if err != nil {
	...
	r.metricsCollector.ObserveControllerReconcileLatency("service", "build_model", buildModelFn)
	if err != nil {
		return errmetrics.NewErrorWithMetrics("service", "build_model_error", err, r.metricsCollector)
	}
	...
	if lb == nil {
		...
		r.metricsCollector.ObserveControllerReconcileLatency("service", "cleanup_load_balancer", cleanupLoadBalancerFn)
		if err != nil {
			return errmetrics.NewErrorWithMetrics("service", "cleanup_load_balancer_error", err, r.metricsCollector)
		}
	}
...
func (r *serviceReconciler) reconcileLoadBalancerResources(ctx context.Context, svc *corev1.Service, stack core.Stack,
	...
	r.metricsCollector.ObserveControllerReconcileLatency("service", "add_finalizers", addFinalizersFn)
	if err != nil {
		...
		return errmetrics.NewErrorWithMetrics("service", "add_finalizers_error", err, r.metricsCollector)
	}

	deployModelFn := func() {
		err = r.deployModel(ctx, svc, stack)
	}
	r.metricsCollector.ObserveControllerReconcileLatency("service", "deploy_model", deployModelFn)
	if err != nil {
	...
	r.metricsCollector.ObserveControllerReconcileLatency("service", "DNS_resolve", dnsResolveFn)
	if err != nil {
		return errmetrics.NewErrorWithMetrics("service", "dns_resolve_error", err, r.metricsCollector)
	}
	...
	if !backendSGRequired {
		if err := r.backendSGProvider.Release(ctx, networking.ResourceTypeService, []types.NamespacedName{k8s.NamespacedName(svc)}); err != nil {
			return errmetrics.NewErrorWithMetrics("service", "release_auto_generated_backend_sg_error", err, r.metricsCollector)
		}
	}
	...
	r.metricsCollector.ObserveControllerReconcileLatency("service", "update_status", updateStatusFn)
	if err != nil {
		...
		return errmetrics.NewErrorWithMetrics("service", "update_status_error", err, r.metricsCollector)
	}
