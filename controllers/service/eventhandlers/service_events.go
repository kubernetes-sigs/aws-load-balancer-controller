...
func NewEnqueueRequestForServiceEvent(eventRecorder record.EventRecorder, annotationParser annotations.Parser,
	loadBalancerClass string, serviceFinalizer string, logger logr.Logger) *enqueueRequestsForServiceEvent {
...
func (h *enqueueRequestsForServiceEvent) isServiceSupported(service *corev1.Service) bool {
	if k8s.HasFinalizer(service, h.serviceFinalizer) {
		return true
	}
	if service.Spec.LoadBalancerClass != nil && *service.Spec.LoadBalancerClass == h.loadBalancerClass {
		return true
	}
	lbType := ""
