...
Instead of depending on IMDSv2, you can specify the AWS Region via the controller flag `--aws-region`, and the AWS VPC via controller flag `--aws-vpc-id` or by specifying vpc tags via the flag `--aws-vpc-tags`. All specified tags are used as AND filters for VPC lookup. When both flags `--aws-vpc-id` and `--aws-vpc-tags` are specified, the controller prioritizes `--aws-vpc-id`and ignores the other flag.
...
2. Download an IAM policy for the LBC using one of the following commands:<p>
    ...
    ```
    curl -o iam-policy.json https://raw.githubusercontent.com/kubernetes-sigs/aws-load-balancer-controller/v3.2.0/docs/install/iam_policy_us-gov.json
    ```
    If your cluster is in a China region:
    ```
    curl -o iam-policy.json https://raw.githubusercontent.com/kubernetes-sigs/aws-load-balancer-controller/v3.2.0/docs/install/iam_policy_cn.json
    ```
    If your cluster is in any other region:
    ```
    curl -o iam-policy.json https://raw.githubusercontent.com/kubernetes-sigs/aws-load-balancer-controller/v3.2.0/docs/install/iam_policy.json
    ```
...
=== "YAML manifests"
    ...
    ```
    wget https://github.com/kubernetes-sigs/aws-load-balancer-controller/releases/download/v3.2.0/v3_2_0_full.yaml
    ```
    ...
    ```
    kubectl apply -f v3_2_0_full.yaml
    ```
    5. Optionally download the default ingressclass and ingressclass params
    ```
    wget https://github.com/kubernetes-sigs/aws-load-balancer-controller/releases/download/v3.2.0/v3_2_0_ingclass.yaml
    ```
    6. Apply the ingressclass and params
    ```
    kubectl apply -f v3_2_0_ingclass.yaml
    ```
