...
| Selector | Meaning |
|---|---|
| `!scooter` | namespaces that do **not** have the `scooter` label key |
| `scooter!=true` | namespaces where `scooter` is unset or set to a value other than `true` |
| `team in (platform,ingress)` | namespaces labeled for those teams |

`!scooter` skips any namespace that carries the `scooter` key (any value). `scooter!=true` is the usual way to skip only `scooter=true` while still watching namespaces that omit the label.
...
An example watching only namespaces without a `scooter` label:

```yaml
spec:
  containers:
  - args:
    - --namespace-selector=!scooter
```
...
| namespace-selector                                                              | string                          |                                            | Label selector for namespaces to watch (standard Kubernetes label selector syntax, e.g. `!scooter`). Mutually exclusive with --watch-namespace. Resolved at startup.           |
