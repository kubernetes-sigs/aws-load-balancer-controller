...
|[sync-period](#sync-period)                            | duration                        | 1h0m0s          | Period at which the controller forces the repopulation of its local object stores|
...
### sync-period
`--sync-period` defines a fix interval for the controller to auto reconcile, default to 1hr. Please be mindful that frequent reconciliations may incur unnecessary AWS API usage.
...
Since no matter how frequent the controller reconciles, there should always be a gap.

### waf-addons
By default, the controller assumes solo ownership of the WAF addons associated to the provisioned ALBs, via the flag `--enable-waf` and `--enable-wafv2`.
...
Once disabled, the controlle shall not take any actions on the waf addons of the provisioned ALBs.
