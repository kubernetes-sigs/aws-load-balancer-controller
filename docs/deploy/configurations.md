...
| aws-vpc-tags                                                                    | stringMap                       |                                            | Tags for the Kubernetes cluster VPC. All specified tags are used as AND filters for VPC lookup. When both flags `--aws-vpc-id` and `--aws-vpc-tags` are specified, the controller prioritizes `--aws-vpc-id` and ignores the other flag. |
| aws-vpc-tag-key                                                                 | string                          | Name                                       | [DEPRECATED] Previously used to select a single tag from `--aws-vpc-tags`. All tags are now always used. This flag will be removed in a future release.                        |
...
| default-pca-arn                                                              | string                          |                   | Default PCA ARN that will be applied to all Ingresses that do not have a PCA ARN set but added the [cerate-acm-cert annotation](../guide/ingress/annotations.md#create-acm-cert)
...
| [route53-validation-record-routing-policy](#route53-validation-record-routing-policy) | string                    | simple                                     | Route53 routing policy to use for ACM DNS validation records this controller creates - simple, weighted                                                                       |
| route53-validation-record-weight                                                | int                             | 100                                        | Weight to use for this controller's ACM DNS validation records when `--route53-validation-record-routing-policy` is `weighted`                                                |
...
| targetgroupbinding-requeue-duration                                             | duration              | 15s                                        | Duration after which TargetGroupBinding will be requeued for reconciliation when it's waiting for AWS resources to update.                                                    |
...
### route53-validation-record-routing-policy
`--route53-validation-record-routing-policy` controls the Route53 routing policy used for the DNS validation records this controller creates for `AMAZON_ISSUED` certificates (see [certificate management](../guide/ingress/certificate_management.md)). It defaults to `simple`.

Set it to `weighted` if you run multiple independent LBC controller deployments that need to create ACM validation records for the same domain without conflicting, for example blue/green clusters cutting over the same hostname. Each controller deployment then uses its own `--cluster-name` as the Route53 `SetIdentifier`, letting both controllers' validation CNAMEs coexist under the same record name.

When deploying via Helm, these flags are exposed as `route53ValidationRecordRoutingPolicy` and `route53ValidationRecordWeight` in `values.yaml`.

`--route53-validation-record-weight` (default `100`) sets the weight assigned to this controller's records when the policy is `weighted`. Only the relative proportion between the weights used by each coexisting controller matters for this use case (any equal, non-zero values work, e.g. `50`/`50` or `100`/`100`). The weight does not need to reflect actual traffic distribution, since it only affects which controller's validation record Route53 returns to a given DNS query, not your application traffic. Avoid setting it to `0`, since Route53 will then never return that controller's record.
