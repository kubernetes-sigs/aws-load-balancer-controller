...
## Packages
- [gateway.k8s.aws/v1](#gatewayk8sawsv1)
...
| --- | --- | --- | --- |
| `type` _[ActionType](#actiontype)_ | The type of action |  | Enum: [forward fixed-response redirect authenticate-cognito authenticate-oidc jwt-validation] <br /> |
...
| `jwtValidationConfig` _[JwtValidationActionConfig](#jwtvalidationactionconfig)_ | Information for a jwt-validation action |  |  |
...
_Validation:_
- Enum: [forward fixed-response redirect authenticate-cognito authenticate-oidc jwt-validation]
...
| `scope` _string_ | The set of user claims to be requested from the IdP. The default is openid .<br />To verify which scope values your IdP supports and how to separate multiple<br />values, see the documentation for your IdP. | openid |  |
...
#### DefaultTargetGroupConfigurationReference
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `name` _string_ | name is the name of the TargetGroupConfiguration resource in the same namespace as this LoadBalancerConfiguration. |  |  |
...
_Validation:_
- Enum: [single-string string-array space-separated-values]

_Appears in:_
- [JwtValidationActionAdditionalClaim](#jwtvalidationactionadditionalclaim)
...
_Appears in:_
- [JwtValidationActionConfig](#jwtvalidationactionconfig)

| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `format` _[JwtAdditionalClaimFormat](#jwtadditionalclaimformat)_ | The format of the claim value(s) |  | Enum: [single-string string-array space-separated-values] <br />Required: \{\} <br /> |
| `name` _string_ | The name of the claim. You can't specify exp, iss, nbf, or iat because we validate them by default. |  | MinLength: 1 <br />Required: \{\} <br /> |
| `values` _string array_ | The claim values. The maximum size of the list is 10. Each value can be up to 256 characters in length.<br />If the format is space-separated-values, the values can't include spaces. |  | MaxItems: 10 <br />MinItems: 1 <br />Required: \{\} <br />items:MaxLength: 256 <br /> |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `issuer` _string_ | The issuer of the JWT. The maximum length is 256 characters. |  | MaxLength: 256 <br />Required: \{\} <br /> |
| `jwksEndpoint` _string_ | The JSON Web Key Set (JWKS) endpoint. This endpoint contains JSON Web Keys (JWK) that are used to validate signatures from the provider.<br />This must be a full URL, including the HTTPS protocol, the domain, and the path. The maximum length is 256 characters. |  | MaxLength: 256 <br />Required: \{\} <br /> |
| `additionalClaims` _[JwtValidationActionAdditionalClaim](#jwtvalidationactionadditionalclaim) array_ | Any additional claims in the JWT that should be validated |  |  |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `protocolPort` _[ProtocolPort](#protocolport)_ | protocolPort is identifier for the listener on load balancer. It should be of the form PROTOCOL:PORT |  | Pattern: `^(HTTP\|HTTPS\|TLS\|TCP\|UDP\|TCP_UDP)?:(6553[0-5]\|655[0-2]\d\|65[0-4]\d\{2\}\|6[0-4]\d\{3\}\|[1-5]\d\{4\}\|[1-9]\d\{0,3\})?$` <br /> |
...
| `targetGroupStickiness` _boolean_ | targetGroupStickiness [Network LoadBalancer] enables sticky routing for requests when using a listener configured with weighted target groups. |  |  |
...
| `quicEnabled` _boolean_ | quicEnabled enables QUIC protocol support for UDP listeners. When enabled, UDP listeners will be upgraded to QUIC protocol. |  |  |
...
| `matchIndexes` _integer_ | Indexes of Match in a rule to apply source ip to<br />If MatchIndexes is not provided, SourceIpConfig will be applied to all conditions where ListenerRuleConfiguration is used |  |  |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `apiVersion` _string_ | `gateway.k8s.aws/v1` | | |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `actions` _[Action](#action) array_ | Actions defines the set of actions to be performed when conditions match.<br />This CRD implementation currently supports only  authenticate-oidc, authenticate-cognito, and fixed-response action types fully and forward and redirect actions partially<br />For other fields in forward and redirect actions, please use the standard Gateway API HTTPRoute or other route resources, which provide<br />native support for those conditions through the Gateway API specification.<br />At most one authentication action can be specified (either authenticate-oidc or authenticate-cognito). |  | MaxItems: 2 <br />MinItems: 1 <br /> |
| `conditions` _[ListenerRuleCondition](#listenerrulecondition) array_ | Conditions defines the circumstances under which the rule actions will be performed.<br />This CRD implementation currently supports only the source-ip condition type<br />For other condition types (such as path-pattern, host-header, http-header, etc.),<br />please use the standard Gateway API HTTPRoute or other route resources, which provide<br />native support for those conditions through the Gateway API specification. |  | MinItems: 1 <br /> |
...
| `kind` _string_ | Kind is the Kubernetes resource kind of the referent. For example<br />"Service".<br />Defaults to "Service" when not specified. | Service |  |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `targetReference` _[Reference](#reference)_ | targetReference the kubernetes object to attach the Target Group settings to.<br />When omitted, this TGC can only be used as a default via LoadBalancerConfiguration's<br />defaultTargetGroupConfiguration reference. |  |  |
...
| `targetControlPort` _integer_ | targetControlPort [Application Load Balancer] The port on which the target communicates its capacity. This value can't be modified after target group creation. |  |  |
