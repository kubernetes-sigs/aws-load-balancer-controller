...
* Installation of Gateway API CRDs — choose one of:
    * **`helm template` (recommended):** render the chart locally and pipe into `kubectl apply --server-side`. This installs CRDs without creating a Helm release secret, bypassing the 1 MB limit that causes `helm install`/`helm upgrade` to fail when CRDs are enabled.
        ...
        ```bash
        helm template aws-load-balancer-controller helm/aws-load-balancer-controller \
    ...
    * **Manual:** apply directly from the upstream release:
        * Standard: `kubectl apply --server-side -f https://github.com/kubernetes-sigs/gateway-api/releases/download/v1.6.2/standard-install.yaml` [REQUIRED]
        * Experimental: `kubectl apply --server-side -f https://github.com/kubernetes-sigs/gateway-api/releases/download/v1.6.2/experimental-install.yaml` [OPTIONAL: Used for L4 Routes]
