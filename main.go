...
func main() {
	...
	lbcMetricsCollector := lbcmetrics.NewCollector(metrics.Registry, mgr, reconcileCounters)
	...
	if metrics.Registry != nil {
		...
		go func() {
			...
			if err := lbcMetricsCollector.StartCollectTopTalkers(ctx); err != nil {
				setupLog.Error(err, "problem periodically collect top talkers")
				os.Exit(1)
			}
		}()
	}
