...
# Copy standard Gateway API CRDs to helm files
COPY_GATEWAY_STANDARD_CRDS_TO_HELM = cp config/crd/gateway/upstream/standard/standard-install.yaml helm/aws-load-balancer-controller/files/standard-gatewayapi-crds.yaml

# Copy experimental Gateway API CRDs to helm files
COPY_GATEWAY_EXPERIMENTAL_CRDS_TO_HELM = cp config/crd/gateway/upstream/experimental/experimental-install.yaml helm/aws-load-balancer-controller/files/experimental-gatewayapi-crds.yaml
...
crds: manifests
	...
	$(KUSTOMIZE) build config/crd/aga >> config/crd/aga/aga-crds.yaml
	$(COPY_GATEWAY_STANDARD_CRDS_TO_HELM)
	$(COPY_GATEWAY_EXPERIMENTAL_CRDS_TO_HELM)
