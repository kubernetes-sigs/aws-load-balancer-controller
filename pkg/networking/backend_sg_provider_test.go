	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sgProvider := NewBackendSGProvider(defaultClusterName, tt.fields.backendSG,
				defaultVPCID, ec2Client, k8sClient, tt.fields.defaultTags, tt.fields.enableGatewayCheck,
				shared_constants.ALBGatewayFinalizer, shared_constants.NLBGatewayFinalizer, logr.New(&log.NullLogSink{}))
