...
type BackendSGProvider interface {
	...
	Get(ctx context.Context, activeResources []metav1.Object) (string, error)
	// Release cleans up the auto-generated backend SG if necessary
	Release(ctx context.Context, activeResources []metav1.Object,
		inactiveResources []metav1.Object, backendSGRequired bool) error
}
...
func (p *defaultBackendSGProvider) Get(ctx context.Context, activeResources []metav1.Object) (string, error) {
	if len(p.backendSG) > 0 {
		return p.backendSG, nil
	}
	// Auto generate Backend Security group, and return the id
	if err := p.allocateBackendSG(ctx, activeResources); err != nil {
...
func (p *defaultBackendSGProvider) Release(ctx context.Context, activeResources []metav1.Object,
	inactiveResources []metav1.Object, backendSGRequired bool) error {
	if len(p.backendSG) > 0 {
		return nil
	}
	defer func() {
		for _, res := range inactiveResources {
			p.objectsMap.Delete(getObjectKey(res))
		}
	}()
	p.updateObjectsMap(ctx, activeResources, inactiveResources, backendSGRequired)
	p.logger.V(1).Info("release backend SG", "active", activeResources,
		"inactive", inactiveResources, "needed", backendSGRequired)
	if len(activeResources) > 0 && backendSGRequired {
		return nil
	}
...
func (p *defaultBackendSGProvider) updateObjectsMap(_ context.Context, activeResources []metav1.Object,
	inactiveResources []metav1.Object, backendSGRequired bool) {
	for _, res := range inactiveResources {
		p.objectsMap.Store(getObjectKey(res), false)
	}
	for _, res := range activeResources {
		p.objectsMap.Store(getObjectKey(res), backendSGRequired)
	}
}
...
func (p *defaultBackendSGProvider) existsInObjectMap(obj metav1.Object) bool {
	if _, exists := p.objectsMap.Load(getObjectKey(obj)); exists {
		return true
	}
	return false
}

func (p *defaultBackendSGProvider) allocateBackendSG(ctx context.Context, activeResources []metav1.Object) error {
	p.mutex.Lock()
	defer p.mutex.Unlock()

	for _, res := range activeResources {
		p.objectsMap.Store(getObjectKey(res), true)
	}
...
func getObjectKey(obj metav1.Object) string {
	return reflect.TypeOf(obj).String() + "/" + k8s.NamespacedName(obj).String()
}
