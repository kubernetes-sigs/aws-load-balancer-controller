...
func (p *defaultBackendSGProvider) syncBackendSGTags(ctx context.Context, sg *ec2sdk.SecurityGroup) error {
	var toChangeTags []*ec2sdk.Tag
	for key, val := range p.defaultTags {
		if !hasTag(sg.Tags, key, val) {
			toChangeTags = append(toChangeTags, &ec2sdk.Tag{
	...
	if len(toChangeTags) == 0 {
		return nil
	}

	sortTags(toChangeTags)
	createReq := &ec2sdk.CreateTagsInput{
		Resources: []*string{sg.GroupId},
		Tags:      toChangeTags,
	}

	_, err := p.ec2Client.CreateTagsWithContext(ctx, createReq)
	if err != nil {
		p.logger.Error(err, "failed to synchronize tags of existing securityGroup", "security group name", sg.GroupName, "security group id", sg.GroupId, "to change tags", toChangeTags)
		return err
	}
	return nil
}
...
func (p *defaultBackendSGProvider) getBackendSGFromEC2(ctx context.Context, sgName string, vpcID string) (*ec2sdk.SecurityGroup, error) {
...
func hasTag(existingTags []*ec2sdk.Tag, key string, value string) bool {
