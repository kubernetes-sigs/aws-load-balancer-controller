...
const (
	defaultWaitALBRegisterPollInterval = 2 * time.Second
	defaultWaitALBRegisterTimeout      = 20 * time.Second
)
...
func (m *frontendNlbTargetsManager) RegisterTargets(ctx context.Context, tgARN string, albTarget elbv2types.TargetDescription) error {
	...
	if err := runtime.RetryImmediateOnError(defaultWaitALBRegisterPollInterval, defaultWaitALBRegisterTimeout, isALbInactiveError, func() error {
