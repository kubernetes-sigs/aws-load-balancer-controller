...
func Test_defaultLoadBalancerManager_removeIPAMPools(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			elbv2Client.EXPECT().ModifyIPPoolsWithContext(gomock.Any(), &elbv2sdk.ModifyIpPoolsInput{RemoveIpamPools: []elbv2types.RemoveIpamPoolEnum{elbv2types.RemoveIpamPoolEnumIpv4}}).Return(&elbv2sdk.ModifyIpPoolsOutput{}, tt.wantErr).Times(tt.wantModify)
...
func Test_defaultLoadBalancerManager_addIPAMPools(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			elbv2Client.EXPECT().ModifyIPPoolsWithContext(gomock.Any(), &elbv2sdk.ModifyIpPoolsInput{IpamPools: &elbv2types.IpamPools{Ipv4IpamPoolId: awssdk.String("bar")}}).Return(&elbv2sdk.ModifyIpPoolsOutput{}, tt.wantErr).Times(tt.wantModify)
