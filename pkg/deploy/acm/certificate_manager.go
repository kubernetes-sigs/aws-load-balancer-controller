...
type CertificateManager interface {
	...
	WaitForCertificateIssuedWithContext(ctx context.Context, arn string, waitTime time.Duration) (*acm.DescribeCertificateOutput, error)

	// GetTags(ctx context.Context, arn string) ([]acmtypes.Tag, error)
}
...
// internal create, wrapped by Create or CreateWithValidationRecords
...
func (c *defaultCertificateManager) WaitForCertificateIssuedWithContext(ctx context.Context, arn string, waitTime time.Duration) (*acm.DescribeCertificateOutput, error) {
	...
	if err != nil {
		return &acm.DescribeCertificateOutput{}, err
	}

	req := &acm.DescribeCertificateInput{
		CertificateArn: awssdk.String(arn),
	}
	cert, err := c.acmClient.DescribeCertificateWithContext(ctx, req)
	if err != nil {
		return &acm.DescribeCertificateOutput{}, err
	}
...
func (c *defaultCertificateManager) DeleteWithValidationRecords(ctx context.Context, arn string) error {
	...
	for _, opts := range desc.Certificate.DomainValidationOptions {
		if opts.ValidationMethod == acmtypes.ValidationMethodDns {
			...
			if strings.Contains(err.Error(), "do not match the current values") {
...
// func (c *defaultCertificateManager) GetTags(ctx context.Context, arn string) ([]acmtypes.Tag, error) {
// 	req := &acm.ListTagsForCertificateInput{
// 		CertificateArn: awssdk.String(arn),
// 	}
//
// 	resp, err := c.acmClient.ListTagsForCertificate(ctx, req)
// 	if err != nil {
// 		return nil, err
// 	}
//
