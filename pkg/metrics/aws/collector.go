	...
	return func(stack *smithymiddleware.Stack) error {
		return stack.Initialize.Add(smithymiddleware.InitializeMiddlewareFunc(sdkMiddlewareCollectAPICallMetric, func(
			...
			if statusCode == "401" || statusCode == "403" {
				c.instruments.apiCallAuthErrorsTotal.With(labels).Inc()
			} else if errorCode == "ServiceLimitExceeded" {
				c.instruments.apiCallLimitExceededTotal.With(labels).Inc()
			} else if errorCode == "ThrottlingException" || errorCode == "Throttling" {
				c.instruments.apiCallThrottledTotal.With(labels).Inc()
			} else if errorCode == "ValidationError" {
				c.instruments.apiCallValidationErrorTotal.With(labels).Inc()
			}
