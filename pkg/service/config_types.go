...
type TargetGroupTuple struct {
	...
	// Whether the target group should use TCP protocol. For TLS listener, target groups default to TLS protocol. To set the target group to TCP protocol. Set TCPEnabled to true.
	// +optional
	TCPEnabled *bool `json:"tcpEnabled,omitempty"`
...
func (c *ForwardActionConfig) validate() error {
	...
	if c.BaseServiceWeight == nil {
		return errors.New("base service weight is required")
	}
	if *c.BaseServiceWeight < 0 || *c.BaseServiceWeight > 999 {
		return errors.New("base service weight must be between 0 and 999")
	}
	return nil
}
