...
func (t *defaultModelBuildTask) buildFrontendNlbTags(ctx context.Context, alb *elbv2model.LoadBalancer) (map[string]string, error) {
	...
	if frontendNlbTags == nil {
		for _, member := range t.ingGroup.Members {
			...
			exists, err := t.annotationParser.ParseStringMapAnnotation(annotations.IngressSuffixTags, &rawTags, member.Ing.Annotations)
			if err != nil {
				return nil, err
			}
			if !exists {
			...
			if frontendNlbTags != nil {
				if !cmp.Equal(frontendNlbTags, rawTags) {
					return nil, errors.Errorf("conflicting ALB tags for frontend NLB: %v | %v", frontendNlbTags, rawTags)
				}
			} else {
				frontendNlbTags = rawTags
			}
		}
	}
