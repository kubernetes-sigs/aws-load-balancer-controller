...
func (t *defaultModelBuildTask) run(ctx context.Context) error {
	...
	if err != nil {
		return errmetrics.NewErrorWithMetrics("ingress", "build_load_balancer_error", err, t.metricsCollector)
	}

	t.sslRedirectConfig, err = t.buildSSLRedirectConfig(ctx, listenPortConfigByPort)
	if err != nil {
		return errmetrics.NewErrorWithMetrics("ingress", "build_ssl_redirct_config_error", err, t.metricsCollector)
	}
	for port, cfg := range listenPortConfigByPort {
		...
		if err != nil {
			return errmetrics.NewErrorWithMetrics("ingress", "build_listener_error", err, t.metricsCollector)
		}
		if err := t.buildListenerRules(ctx, ls.ListenerARN(), port, cfg.protocol, ingList); err != nil {
			return errmetrics.NewErrorWithMetrics("ingress", "build_listener_rule_error", err, t.metricsCollector)
		}
	}

	if err := t.buildLoadBalancerAddOns(ctx, lb.LoadBalancerARN()); err != nil {
		return errmetrics.NewErrorWithMetrics("ingress", "build_load_balancer_addons", err, t.metricsCollector)
	}
	return nil
}
