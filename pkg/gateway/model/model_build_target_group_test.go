...
func Test_buildTargetGroupTags(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			builder := newTargetGroupBuilder("test-cluster", "vpc-xxx", tagger, elbv2model.LoadBalancerTypeApplication, gateway.NewTargetGroupConfigConstructor(), false, string(elbv2model.TargetTypeIP), nil)
