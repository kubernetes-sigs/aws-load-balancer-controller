...
func serviceLoader(ctx context.Context, k8sClient client.Client, routeIdentifier types.NamespacedName, routeKind RouteKind, backendRef gwv1.BackendRef, gatewayDefaultTGConfig *elbv2gw.TargetGroupConfiguration) (*ServiceBackendConfig, error, error) {
	...
	if svcNamespace != routeIdentifier.Namespace {
		allowed, err := shared_utils.ValidateCrossNamespaceReference(ctx, k8sClient, routeIdentifier.Namespace, gatewayAPIGroup, string(routeKind), coreAPIGroup, serviceKind, svcIdentifier.Namespace, svcIdentifier.Name)
		if err != nil {
