...
type listenerToRouteMapper interface {
	mapGatewayAndRoutes(context context.Context, gw gwv1.Gateway, listeners []DiscoveredListener, routes []preLoadRouteDescriptor) (map[int][]preLoadRouteDescriptor, map[int32]map[string][]gwv1.Hostname, []RouteData, map[string][]gwv1.ParentReference, map[gwv1.SectionName]int32, error)
}
...
func (ltr *listenerToRouteMapperImpl) mapGatewayAndRoutes(ctx context.Context, gw gwv1.Gateway, listeners []DiscoveredListener, routes []preLoadRouteDescriptor) (map[int][]preLoadRouteDescriptor, map[int32]map[string][]gwv1.Hostname, []RouteData, map[string][]gwv1.ParentReference, map[gwv1.SectionName]int32, error) {
	...
	// Track route attachment to each listener.
	routesPerSectionName := make(map[gwv1.SectionName]int32)
	...
	for _, discoveredListener := range listeners {
		...
		routesPerSectionName[listener.Name] = 0
		...
		for _, route := range routesForGateway {
			...
			if allowedAttachment {
				routesPerSectionName[listener.Name] = routesPerSectionName[listener.Name] + 1
	...
	return result, compatibleHostnamesByPort, failedRoutes, matchedParentRefsResult, routesPerSectionName, nil
}
