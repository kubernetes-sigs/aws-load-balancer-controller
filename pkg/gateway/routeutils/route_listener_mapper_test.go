...
func Test_mapGatewayAndRoutes(t *testing.T) {
	...
	testCases := []struct {
		...
		sectionRouteCount     map[gwv1.SectionName]int32
		expectErr             bool
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, compatibleHostnames, statusUpdates, _, sectionRouteCount, err := mapper.mapGatewayAndRoutes(context.Background(), tc.gw, discoverListeners(tc.gw), tc.routes)
			...
			assert.Equal(t, tc.sectionRouteCount, sectionRouteCount)
