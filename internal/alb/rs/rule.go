...
func (r *Rule) Reconcile(ctx context.Context, rOpts *ReconcileOptions) error {
	...
	if r.rs.desired != nil {
		for i := range r.rs.desired.Actions {
			...
			r.rs.desired.Actions[i].TargetGroupArn = r.TargetGroupArn(ctx, rOpts.TargetGroups)
		}
	}
	...
	case r.rs.desired == nil: // rule should be deleted
		...
		albctx.GetLogger(ctx).Infof("Start Rule deletion.")
		if err := r.delete(ctx, rOpts); err != nil {
			return err
		}
		albctx.GetEventf(ctx)(api.EventTypeNormal, "DELETE", "%s rule deleted", *r.rs.current.Priority)
		albctx.GetLogger(ctx).Infof("Completed Rule deletion. Rule Priority: %s | Condition: %s",
	...
	case r.rs.current == nil: // rule doesn't exist and should be created
		albctx.GetLogger(ctx).Infof("Start Rule creation.")
		if err := r.create(ctx, rOpts); err != nil {
			return err
		}
		albctx.GetEventf(ctx)(api.EventTypeNormal, "CREATE", "%s rule created", *r.rs.current.Priority)
		albctx.GetLogger(ctx).Infof("Completed Rule creation. Rule Priority: %s | Condition: %s",
			log.Prettify(r.rs.current.Priority),
			log.Prettify(r.rs.current.Conditions))

	case r.needsModification(ctx): // diff between current and desired, modify rule
		if err := r.modify(ctx, rOpts); err != nil {
			return err
		}
		albctx.GetEventf(ctx)(api.EventTypeNormal, "MODIFY", "%s rule modified", *r.rs.current.Priority)
	}

	return nil
}

func (r *Rule) TargetGroupArn(ctx context.Context, tgs tg.TargetGroups) *string {
	i := tgs.LookupByBackend(extensions.IngressBackend{ServiceName: r.svc.desired.name, ServicePort: r.svc.desired.port})
	if i < 0 {
		albctx.GetLogger(ctx).Errorf("Failed to locate TargetGroup related to this service: %s:%s", r.svc.desired.name, r.svc.desired.port.String())
		return nil
	}
	arn := tgs[i].CurrentARN()
	if arn == nil {
		albctx.GetLogger(ctx).Errorf("Located TargetGroup but no known (current) state found: %s:%s", r.svc.desired.name, r.svc.desired.port.String())
	}
	return arn
}

func (r *Rule) create(ctx context.Context, rOpts *ReconcileOptions) error {
	...
	if err != nil {
		albctx.GetEventf(ctx)(api.EventTypeWarning, "ERROR", "Error creating %v rule: %s", *in.Priority, err.Error())
...
func (r *Rule) modify(ctx context.Context, rOpts *ReconcileOptions) error {
...
func (r *Rule) delete(ctx context.Context, rOpts *ReconcileOptions) error {
	if r.rs.current == nil {
		albctx.GetLogger(ctx).Infof("Rule entered delete with no Current to delete. Rule: %s", log.Prettify(r))
		return nil
	}

	// If the current rule was a default, it's bound to the listener and won't be deleted from here.
	if *r.rs.current.IsDefault {
		albctx.GetLogger(ctx).Debugf("Deletion hit for default rule, which is bound to the Listener. It will not be deleted from here. Rule. Rule: %s",
			log.Prettify(r))
		return nil
	}
	...
	if _, err := albelbv2.ELBV2svc.DeleteRule(in); err != nil {
		albctx.GetEventf(ctx)(api.EventTypeWarning, "ERROR", "Error deleting %s rule: %s", *r.rs.current.Priority, err.Error())
...
func (r *Rule) needsModification(ctx context.Context) bool {
	crs := r.rs.current
	drs := r.rs.desired
	...
	case !util.DeepEqual(crs.Actions, drs.Actions):
		albctx.GetLogger(ctx).Debugf("Actions needs to be changed (%v != %v)", log.Prettify(crs.Actions), log.Prettify(drs.Actions))
		return true
	case !conditionsEqual(crs.Conditions, drs.Conditions):
		albctx.GetLogger(ctx).Debugf("Conditions needs to be changed (%v != %v)", log.Prettify(crs.Conditions), log.Prettify(drs.Conditions))
		return true
	case r.svc.current.name != r.svc.desired.name && r.svc.current.port.String() != action.UseActionAnnotation:
		albctx.GetLogger(ctx).Debugf("SvcName needs to be changed (%v != %v)", r.svc.current.name, r.svc.desired.name)
		return true
	case r.svc.current.port.String() != r.svc.desired.port.String():
		albctx.GetLogger(ctx).Debugf("SvcPort needs to be changed (%v != %v)", r.svc.current.port.String(), r.svc.desired.port.String())
		return true
	}
