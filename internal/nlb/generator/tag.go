...
func (gen *TagGenerator) TagTG(serviceName string) map[string]string {
	return map[string]string{
		TagKeyServiceName: serviceName,
	}
}

func (gen *TagGenerator) TagLBSG(namespace string, serviceName string) map[string]string {
	return gen.tagSGs(namespace, serviceName)
}

func (gen *TagGenerator) TagInstanceSG(namespace string, serviceName string) map[string]string {
	return gen.tagSGs(namespace, serviceName)
}
...
// Tagging for sg is optional since service controller used name to resolve tags, but it will be required when
// * add support to clean up aws resources created by service controller
// * add support for sharing instance securityGroup among servicees.

func (gen *TagGenerator) tagSGs(namespace string, serviceName string) map[string]string {
	m := make(map[string]string)
	for label, value := range gen.DefaultTags {
		m[label] = value
	}
