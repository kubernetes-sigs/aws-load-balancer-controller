...
import (
	...
	"github.com/kubernetes-sigs/aws-alb-ingress-controller/internal/nlb/sg"
...
func NewController(
	...
	attrsController := lb.NewAttributesController(cloud)
...
type defaultController struct {
	...
	attrsController         lb.AttributesController
}
...
func (controller *defaultController) Reconcile(ctx context.Context, service *corev1.Service) (*LoadBalancer, error) {
	...
	if controller.store.GetConfig().FeatureGate.Enabled(config.WAF) {
		if err := controller.reconcileWAF(ctx, lbArn, serviceAnnos.LoadBalancer.WebACLId); err != nil {
			return nil, err
		}
	}
	...
	if err := controller.sgAssociationController.Reconcile(ctx, service, instance, tgGroup); err != nil {
...
func (controller *defaultController) Delete(ctx context.Context, key types.NamespacedName) error {
	...
	if instance != nil {
		if err = controller.sgAssociationController.Delete(ctx, key, instance); err != nil {
...
func (controller *defaultController) buildLBConfig(ctx context.Context, service *corev1.Service, serviceAnnos *annotations.Service) (*loadBalancerConfig, error) {
	...
	lbType := serviceAnnos.LoadBalancer.Type
	if aws.StringValue(lbType) == "" {
		lbType = aws.String(elbv2.LoadBalancerTypeEnumApplication)
	}
