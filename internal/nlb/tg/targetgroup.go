...
func (controller *defaultController) Reconcile(ctx context.Context, service *corev1.Service, backend extensions.IngressBackend) (TargetGroup, error) {
	...
	healthCheckPort, err := controller.resolveServiceHealthCheckPort(service.GetNamespace(), backend.ServiceName, intstr.Parse(*serviceAnnos.HealthCheck.Port), targetType)

	if err != nil {
		return TargetGroup{}, fmt.Errorf("failed to resolve healthcheck port due to %v", err)
	}

	tgName := controller.nameTagGen.NameTG(service.GetNamespace(), backend.ServiceName, targetType, protocol)
...
func (controller *defaultController) buildTags(meta metav1.Object, backend extensions.IngressBackend, serviceAnnos *annotations.Service) map[string]string {
	tgTags := make(map[string]string)
	for k, v := range controller.nameTagGen.TagTGGroup(meta.GetNamespace(), meta.GetName()) {
		tgTags[k] = v
	}
	for k, v := range controller.nameTagGen.TagTG(backend.ServiceName) {
		tgTags[k] = v
	}
