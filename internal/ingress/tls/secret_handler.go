...
//TODO, if this pattern proves to be useful, it can be shared with auth/secret_handler via some common libs.
...
func (h *EnqueueRequestsForSecretEvent) enqueueImpactedObjects(secret *corev1.Secret, _ workqueue.RateLimitingInterface) {
	...
	if err := h.Cache.List(context.TODO(), client.MatchingField(FieldTLSSecret, secretKey), ingressList); err != nil {
		glog.Errorf("failed to fetch impacted ingresses by %v due to %v", FieldTLSSecret, err)
		return
	}
